...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, vehicleFilename, playerId, ksi, lat, speedUpdate, id, playingId):
        super().__init__()
        ...
        self.image = pygame.transform.rotate(self.imageInit, track.angles[0] * 180 / math.pi)
        ...
        self.latUpdate = [0, 0, 0, 0]  # Lateral position update parameters
        self.speed = 0  # Vehicle speed
        self.speedUpdate = speedUpdate  # Speed update parameters
        vehicleCoords = natToGlobal(track, self.seg, self.ksi, self.lat)
        self.rect.center = (vehicleCoords[0], vehicleCoords[1])  # Initial position
        self.dir = vehicleCoords[2]  # Initial direction (rad)
    ...
    def external_update(self, data):
        ...
        """
        self.seg = data.get('seg', self.seg)
        self.ksi = data.get('ksi', self.ksi)
        self.lat = data.get('lat', self.lat)
        self.speed = data.get('speed', self.speed)
        self.dir = data.get('dir', self.dir)
        self.distance = data.get('distance', self.distance)
        self.lap = data.get('lap', self.lap)
        
        # Update the vehicle's rect position based on the new coordinates
        if 'x' in data and 'y' in data:
            self.rect.center = (data['x'], data['y'])
        
        # Ensure the vehicle's image is correctly oriented
        self.image = pygame.transform.rotate(self.imageInit, -self.dir * 180 / math.pi)
...
class Player():
    def __init__(
            self, playerId, 
            vehicleGroup: pygame.sprite.Group, vessel: Vessel, 
...
track = Track(TRACK_SEG_TYPES, TRACK_TRANS_POINTS, TRACK_ARC_DATA, TRACK_ANGLES, TRACK_ARC_ORIENT, TRACK_KSI)
vesselGroup = pygame.sprite.Group()  # All vehicle and vessel sprites
fruitGroup = pygame.sprite.Group()  # All projectile sprites
